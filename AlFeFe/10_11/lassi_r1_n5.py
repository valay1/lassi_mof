...
r=1;n=5
output='r'+str(r)+'_n'+str(n)+'_analyze.log'
...
ci1, si1, space_weights, navg, maxw, entr = analyze(las,si, state=[0,1,2,3,4],return_metrics=True)
np.save('space_weights',space_weights)
np.save('navg',navg)
np.save('maxw',maxw)
np.save('entr',entr)
